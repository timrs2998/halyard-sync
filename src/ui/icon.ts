/**
 * Tether Sync's custom icon: two anchored nodes joined by a tether cord,
 ...
 * with a class hook (`tether-sync-icon-cord`) so `styles.css` can animate
...
export const TETHER_SYNC_ICON_ID = "tether-sync-cord";

export function registerTetherSyncIcon(): void {
	addIcon(
		TETHER_SYNC_ICON_ID,
		`<circle cx="26" cy="26" r="9" fill="currentColor" stroke="none"/>` +
			`<circle cx="74" cy="74" r="9" fill="currentColor" stroke="none"/>` +
			`<path class="tether-sync-icon-cord" d="M30 34 C 55 34, 45 66, 70 66" ` +
